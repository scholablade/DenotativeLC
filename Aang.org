...
#+RESULTS:
: <interactive>:603:4-60: error:
:     • The type family ‘Curry’ should have 3 arguments, but has been given 1
:     • In the type synonym declaration for ‘Abstraction’


#+begin_src haskell

-- type family  A v where
--   A v =  MkReader (UnReader v <=< Fst)
-- data Curry :: ((a, b) -> c) -> a -> b -> c

-- type instance Eval (Curry f x y) = Eval (f '(x, y))
type family A a where
  A a = Curry (UnReader a)
-- (.) :: (b -> c) -> (a -> b) -> a -> c
-- data Compose :: (a -> Exp b) -> (b -> Exp c) -> a -> Exp c
-- type instance Eval (Compose f g x) = f (g x)
#+End_src

#+RESULTS:
: ghci| ghci| 
: <interactive>:627:3-26: error:
:     • The type family ‘Curry’ should have 3 arguments, but has been given 1
:     • In the equations for closed type family ‘A’
